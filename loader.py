...
class ct_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        input_img, target_img = self.input_[idx], self.target_[idx] #peidui dijilianghe zhengchangjiliang de peidui 
        if self.load_mode == 0:
        ...
        if self.patch_size:
            input_patches, target_patches = get_patch(input_img,
                                                      ...
                                                      self.patch_n,
                                                      self.patch_size)
            return (input_patches, target_patches) #dedao jiyu kuai de dijlianghezhengchangjiliang
        else:
