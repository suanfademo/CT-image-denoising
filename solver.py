...
from networks import EICNN
from measure import compute_measure,compute_PSNR
...
class Solver(object):
    def __init__(self, args, data_loader):
        ...
        if (self.multi_gpu) and (torch.cuda.device_count() > 1):
            ...
            self.EICNN = nn.DataParallel(self.EICNN)
        self.EICNN.to(self.device)

        self.lr = args.lr
        self.criterion = nn.MSELoss()
        self.optimizer = optim.Adam(self.EICNN.parameters(), self.lr)


    def save_model(self, iter_):
        f = os.path.join(self.save_path, 'EICNN_{}iter.ckpt'.format(iter_))
        torch.save(self.EICNN.state_dict(), f)
    ...
    def load_model(self, iter_):
        ...
        if self.multi_gpu:
            ...
            for k, v in torch.load(f):
                n = k[7:]
                state_d[n] = v
            self.EICNN.load_state_dict(state_d)
        else:
            self.EICNN.load_state_dict(torch.load(f))
